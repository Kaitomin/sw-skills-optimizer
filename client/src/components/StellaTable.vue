<template>
  ...
  <div class="skills-details">
    <div v-if="char">
      ...
      <div class="stella-buff">
        <div>
          <img
            src="/stella_ghost.png"
            alt=""
            width="40"
            height="40"
          />
        </div>
        <button class="stella-ghost" @click="toggleGhost">Ghost buff</button>
      </div>
      ...
      <table v-if="clientWidth > 550" class="table table-striped table-skills">
        ...
        <tbody>
          <tr v-for="skill in skillsTable" :key="skill._id">
            ...
            <td class="dps">
              {{
                castChecked && stellaGhost ? 
                  Math.round(skill.dmgGhost / (skill.castCancel / 60)) : 
                  castChecked ? 
                    Math.round(skill.dmg / (skill.castCancel / 60)) :
                    stellaGhost ? 
                      Math.round(skill.dmgGhost / (skill.cast / 60)) :
            ...
            <td>
              {{
                stellaGhost ? 
                  Math.round(skill.dmgGhost / calcCD(skill)) : 
            ...
            <td>
              {{
                stellaGhost ? 
                  Math.round(skill.dmgGhost / calcCD15(skill)) : 
...
export default {
  ...
  methods: {
    ...
    toggleGhost() {
      this.stellaGhost = !this.stellaGhost

      if (this.stellaGhost) {
        // Disable Ephnel Release checkbox
        document
          .querySelector(".stella-ghost")
          .classList.add("active-buff")

        this.$emit("stella-dmg", "ghost")
      } else {
        // Disable Ephnel Release checkbox
        document
          .querySelector(".stella-ghost")
        ...
        this.$emit("stella-dmg", "")
      }
    },
    calcDmg(skill) {
      if (this.stellaGhost) return skill.dmgGhost
      return skill.dmg
    },
    calcCD(skill) {
    ...
    sortBy(criteria) {
      ...
      switch (criteria) {
        case "dmg": {
          if (this.stellaGhost) {
            this.skillsTable.sort((a, b) =>
              this.sortOrder ? 
                +a.dmgGhost - +b.dmgGhost : 
                +b.dmgGhost - +a.dmgGhost
            )
          } else {
            this.skillsTable.sort((a, b) =>
              this.sortOrder ?
        ...
        case "dps":
          if (this.castChecked && this.stellaGhost) {
            this.skillsTable.sort((a, b) =>
              this.sortOrder ? 
                Math.round(a.dmgGhost / (a.castCancel / 60)) - Math.round(b.dmgGhost / (b.castCancel / 60)) : 
                Math.round(b.dmgGhost / (b.castCancel / 60)) - Math.round(a.dmgGhost / (a.castCancel / 60))
            )
          ...
          } else if (this.stellaGhost) {
            this.skillsTable.sort((a, b) =>
              this.sortOrder ? 
                Math.round(a.dmgGhost / (a.cast / 60)) - Math.round(b.dmgGhost / (b.cast / 60)) :
                Math.round(b.dmgGhost / (b.cast / 60)) - Math.round(a.dmgGhost / (a.cast / 60))
            )
          } else {
            this.skillsTable.sort((a, b) =>
              this.sortOrder ? 
        ...
        case "dmg-cd":
          if (this.stellaGhost) {
            this.skillsTable.sort((a, b) =>
              this.sortOrder ? 
                Math.round(a.dmgGhost / this.calcCD(a)) - Math.round(b.dmgGhost / this.calcCD(b)) : 
                Math.round(b.dmgGhost / this.calcCD(b)) - Math.round(a.dmgGhost / this.calcCD(a))
            )  
          } else {
            this.skillsTable.sort((a, b) =>
              this.sortOrder ? 
        ...
        case "dmg-cd15":
          if (this.stellaGhost) {
            this.skillsTable.sort((a, b) =>
              this.sortOrder ? 
                Math.round(a.dmgGhost / this.calcCD15(a)) - Math.round(b.dmgGhost / this.calcCD15(b)) : 
                Math.round(b.dmgGhost / this.calcCD15(b)) - Math.round(a.dmgGhost / this.calcCD15(a))
            )
          } else {
            this.skillsTable.sort((a, b) =>
              this.sortOrder ? 
  ...
  created() {
    CharacterService.getCharacterInfo(this.charName)
      .then(res => {
        ...
        this.skillsTable.forEach(skill => {
          ...
          skill.dmgGhost = Math.round(skill.dmgGhost * 1.2)
        })
        this.displayTooltip;
    ...
    this.description = "Data by sweepy from EN [26/06/2024]";
    this.aspd = 240;
  }
};
</script>
...
.stella-buff button:hover {
  box-shadow: 0 0 0 1px white;
  background-color: #ffffff1a;
  border-radius: 5px
}
.stella-buff button.active-buff {
  background-color: #441189;
